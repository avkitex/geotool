...
def _ebayes(
    ...
    t2 = out["t"] ** 2
    # np.asarray(...) -- not upstream, see ../README.md. When df_prior is
    # scalar (the non-robust, no-covariate case) and equal to np.inf, this
    # comparison is plain-Python-float > plain-Python-float, giving a
    # native Python bool rather than a numpy bool_. `~` on a native bool
    # inverts it as an int (~True == -2), not a logical negation, which
    # then gets used as a column position below -- wrapping in np.asarray
    # keeps it a proper (0-d) numpy bool array, where `~` means what the
    # surrounding code assumes it means.
    Infdf = np.asarray(out["df_prior"] > 1e6)
    if np.any(Infdf):
        kernel = t2 * (1 - 1 / r) / 2
        if np.any(~Infdf):
            t2_f = t2[~Infdf]
